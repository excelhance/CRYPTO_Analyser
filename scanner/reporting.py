"""Restitution du scan : console (rich) + CSV (§6 CDC) + prompt fondamental (§5, Lot 5).

Ne connaît rien du fetch/scoring/fondamental : consomme uniquement un `ScanResult`
(Lot 4, `scanner.py`) ou un `FundamentalsPromptResult` (Lot 5, `fundamentals.py`).
...
from .fundamentals import FundamentalsPromptResult
...
# --------------------------------------------------------------------------- #
# Prompt fondamental (§5 CDC, Lot 5, Mode A) : prêt à coller dans Claude        #
# --------------------------------------------------------------------------- #
def write_fundamentals_prompt(result: FundamentalsPromptResult, config: AppConfig) -> Path:
    """Écrit le prompt fondamental horodaté (le contenu du fichier EST le prompt,
    tel quel, pour rester copiable/collable sans nettoyage manuel).
    ...
    directory.mkdir(parents=True, exist_ok=True)
    stem = result.generated_at.strftime("fundamentals_prompt_%Y%m%d_%H%M")
